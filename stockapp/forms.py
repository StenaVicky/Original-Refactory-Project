...
class StockReceiptForm(PlainFormMixin, forms.ModelForm):
    class Meta:
        model = StockReceipt
    ...
    def clean_supplier_name(self):
        ...
        if not name or len(name) < 3:
            raise forms.ValidationError('Supplier name must be at least three characters')
        return name


class ProductForm(PlainFormMixin, forms.ModelForm):
    class Meta:
        model = Product
